# =============================================================================
# weighted_ensemble.py - Weighted Ensemble of V1, V2, V3
# وزن‌ها: V1=0.6, V2=0.3, V3=0.1 (قابل تنظیم)
# =============================================================================
import torch
import numpy as np
import pandas as pd
...
print(f"⚖️  Weights: V1={W1}, V2={W2}, V3={W3}")
...
print("=" * 60)
print(f"WEIGHTED ENSEMBLE: V1={W1}, V2={W2}, V3={W3}")
print("=" * 60)
...
def evaluate_ensemble(loader, tta=4, threshold=0.5):
    ...
    for x, y in loader:
        ...
        with autocast():
            ...
            logits2 = model_v2(x)
            logits3 = model_v3(x)
            weighted_logits = W1 * logits1 + W2 * logits2 + W3 * logits3
            probs = torch.softmax(weighted_logits, dim=1)[:, 1].cpu().numpy()
...
print("
" + "=" * 60)
print("🏆 WEIGHTED ENSEMBLE (V1+V2+V3) FINAL RESULTS — TEST SET")
print("=" * 60)
print(f"Weights:                 V1={W1}, V2={W2}, V3={W3}")
...
results_df = pd.DataFrame([{
    "model": f"Weighted Ensemble (V1={W1}, V2={W2}, V3={W3})",
...
results_df.to_csv(RESULTS_DIR / "weighted_ensemble_test_results.csv", index=False)
print(f"
💾 Results saved to: {RESULTS_DIR / 'weighted_ensemble_test_results.csv'}")
