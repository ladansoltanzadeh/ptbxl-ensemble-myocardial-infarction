# =============================================================================
# ensemble_teacher.py - Ensemble of wider_transformer (v1) and wider_transformer_v2
# =============================================================================
import torch
import numpy as np
import pandas as pd
...
def find_best_v2_checkpoint(v2_dir):
    history_path = v2_dir / "history.csv"
    if not history_path.exists():
        raise FileNotFoundError(f"history.csv not found in {v2_dir}")
    df = pd.read_csv(history_path)
    best_row = df.loc[df["val_accuracy"].idxmax()]
    best_epoch = int(best_row["epoch"])
    ckpt_path = v2_dir / f"checkpoint_epoch_{best_epoch}.pt"
    if not ckpt_path.exists():
        raise FileNotFoundError(f"Checkpoint not found: {ckpt_path}")
    print(f"✅ Found best V2 checkpoint: epoch {best_epoch} (Acc: {best_row['val_accuracy']:.2f}%)")
    return ckpt_path, best_row["val_accuracy"]

print("=" * 60)
print("ENSEMBLE: wider_transformer (v1) + wider_transformer_v2")
print("=" * 60)

if not V1_CKPT.exists():
    raise FileNotFoundError(f"❌ V1 checkpoint not found: {V1_CKPT}")
...
v2_ckpt_path, v2_acc = find_best_v2_checkpoint(V2_DIR)
ckpt_v2 = torch.load(v2_ckpt_path, map_location="cpu")
config_v2 = config_v1
model_v2 = TeacherBinary(
    num_classes=2,
    transformer_layers=config_v2["transformer_layers"],
    hidden_dim=config_v2["transformer_hidden"],
    dropout=config_v2["dropout"],
).to(device)
...
print("
" + "=" * 60)
print("🏆 ENSEMBLE FINAL RESULTS — TEST SET")
print("=" * 60)
...
results_df.to_csv(RESULTS_DIR / "ensemble_test_results.csv", index=False)
print(f"
💾 Results saved to: {RESULTS_DIR / 'ensemble_test_results.csv'}")
